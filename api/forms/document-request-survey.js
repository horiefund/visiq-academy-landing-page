...
async function airtableFindRecordByEmail({ baseId, apiKey, tableName, emailFieldName, email }) {
...
async function resolveParentMasterRecordId({ baseId, apiKey, fields }) {
  ...
  sendDebugLog({
    runId: "run-3",
    hypothesisId: "H7",
    location: "api/forms/document-request-survey.js:resolveParentMasterRecordId:88",
    message: "Resolving parent master link by email",
    data: { hasEmail: Boolean(email) },
  });

  const findResult = await airtableFindRecordByEmail({
    baseId,
    apiKey,
  ...
  if (!findResult.ok) {
    const errorMessage = findResult.data?.error?.message || "Failed to find parent master record";
  ...
  const existingRecordId = findResult.data?.records?.[0]?.id;
  if (existingRecordId) {
    sendDebugLog({
      runId: "run-3",
      hypothesisId: "H7",
      location: "api/forms/document-request-survey.js:resolveParentMasterRecordId:110",
      message: "Matched existing parent master record",
      data: { parentMasterRecordId: existingRecordId },
    });
  ...
  sendDebugLog({
    runId: "run-3",
    hypothesisId: "H7",
    location: "api/forms/document-request-survey.js:resolveParentMasterRecordId:126",
  ...
  if (!createResult.ok) {
    const errorMessage = createResult.data?.error?.message || "Failed to create parent master record";
    const error = new Error(errorMessage);
    error.statusCode = 502;
  ...
  sendDebugLog({
    runId: "run-3",
    hypothesisId: "H7",
    location: "api/forms/document-request-survey.js:resolveParentMasterRecordId:156",
...
function normalizeFields(rawFields) {
  ...
  const concerns = fields["気になっていること"];

  if (typeof concerns === "string" && concerns.trim() !== "") {
    fields["気になっていること"] = [concerns.trim()];
  } else if (Array.isArray(concerns)) {
    fields["気になっていること"] = concerns.filter((item) => typeof item === "string" && item.trim() !== "").map((item) => item.trim());
  }
...
async function handler(req, res) {
  ...
  try {
    ...
    sendDebugLog({
      runId: "run-1",
      hypothesisId: "H2",
      location: "api/forms/document-request-survey.js:handler:63",
      message: "API handler entered",
      data: {
        method: req.method,
        requestTableName: requestTableName || null,
        fieldKeys: Object.keys(fields || {}),
        concernsType: Array.isArray(fields["気になっていること"]) ? "array" : typeof fields["気になっていること"],
      },
    });
    ...
    if (!validateFields(fields)) {
      sendDebugLog({
        runId: "run-1",
        hypothesisId: "H1",
        location: "api/forms/document-request-survey.js:validateFields:67",
    ...
    sendDebugLog({
      runId: "run-2",
      hypothesisId: "H6",
      location: "api/forms/document-request-survey.js:envCheck:121",
    ...
    if (!enableAirtable) {
      sendDebugLog({
        runId: "run-2",
        hypothesisId: "H6",
        location: "api/forms/document-request-survey.js:mockBranch:140",
        message: "Returned mock mode due to environment flag",
        data: {
          enableAirtableRaw: enableAirtableRaw ?? null,
          enableAirtable,
        },
      ...
      return res.status(200).json({
        ok: true,
        mode: "mock",
        received: { tableName: TABLE_NAME, fields },
      });
    }
    ...
    sendDebugLog({
      runId: "run-3",
      hypothesisId: "H8",
      location: "api/forms/document-request-survey.js:beforeSurveyCreate:220",
      message: "Prepared fields before survey create",
      data: {
    ...
    sendDebugLog({
      runId: "run-1",
      hypothesisId: "H3",
      location: "api/forms/document-request-survey.js:airtableCreateRecord:105",
      ...
      data: {
        ...
        returnedFieldKeys: Object.keys(result?.data?.records?.[0]?.fields || {}),
        returnedLinkField: result?.data?.records?.[0]?.fields?.[PARENT_MASTER_LINK_FIELD_NAME] || null,
      },
    });
    if (!result.ok) {
      const errorMessage = result.data?.error?.message || "Airtable request failed";
    ...
    console.error("[document-request-survey] unexpected error", error);
    sendDebugLog({
      runId: "run-1",
      hypothesisId: "H5",
      location: "api/forms/document-request-survey.js:catch:136",
      message: "Unhandled exception in API route",
      data: {
        errorMessage: String(error?.message || error),
        errorName: error?.name || null,
        errorStage: error?.stage || null,
        errorStackTop: typeof error?.stack === "string" ? error.stack.split("\n").slice(0, 3).join(" | ") : null,
      },
    });
